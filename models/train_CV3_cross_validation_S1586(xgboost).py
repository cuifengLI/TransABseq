...
def cross_validation(train_esm, train_prot, train_label, valid_esm, valid_prot, valid_label,
    ...
    plt.title(f'TEST{i} Training And Validation LOSS')
    ...
    print(f"Validation {i} - Validation Results (XGB):")
    evaluate_regression(val_pred_xgb, valid_label, save_to_file=False)
    print(f"Test {i} - Testing Results (XGB):")
...
if __name__ == "__main__":
    ...
    for i in range(1, 21):
        train_ESM, test_ESM, train_Prot, test_Prot, train_Y, test_Y = train_test_split(all_esm, all_prot, all_label,
